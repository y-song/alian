...
void plot_data_eta(std::string file_name)
{
    SetStyle();
    ...
    TH1D *h_JetEta_pre_select = (TH1D *)f->Get("jet_eta");
    TH1D *h_JetEta = (TH1D *)f->Get("jet_eta_post_selection");
    ...
    FormatHist(h_JetEta_pre_select, kBlue, kFullSquare);
    // FormatHist(h_JetPt, kBlue, kFullTriangleUp);
    FormatHist(h_JetEta, kRed, kFullCircle);
    h_JetEta_pre_select->DrawNormalized();
    // h_JetPt->Draw("same");
    h_JetEta->DrawNormalized("same");

    c->SaveAs("output/data_eta.png");
}